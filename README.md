...
Задача про разрабокту парсера регулярных выражений (в том формате, в котором их видит автор).
Было очень легко, потому что недавно у меня начался курс разработки компиляторов - я просто скопировал первую лабораторную
работу по парсингу регулярок и даже не стал передеывать, просто дописал маппинг в типы по задаче.
